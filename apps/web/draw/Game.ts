...
export class Game {
  ...
  private resizeGroup(x: number, y: number, e: MouseEvent) {
    ...
    for (const originalShape of this.originalShapes.values()) {
      ...
      if (shape && shape.shape) {
        ...
        const relativeX =
          (this.getShapePosition(originalShape).x - originalBBox.minX) /
          originalWidth;
        const relativeY =
          (this.getShapePosition(originalShape).y - originalBBox.minY) /
          originalHeight;

        const newX =
          originalBBox.minX +
          translateX +
          (this.getShapePosition(originalShape).x - originalBBox.minX) * scaleX;
        const newY =
          originalBBox.minY +
          translateY +
          (this.getShapePosition(originalShape).y - originalBBox.minY) * scaleY;
        ...
        switch (s.type) {
          ...
          case "line":
            ...
            const originalStartX =
              (originalLine.x - originalBBox.minX) / originalWidth;
            const originalStartY =
              (originalLine.y - originalBBox.minY) / originalHeight;
            const originalEndX =
              (originalLine.x2 - originalBBox.minX) / originalWidth;
            const originalEndY =
              (originalLine.y2 - originalBBox.minY) / originalHeight;

            s.x =
              originalBBox.minX +
              translateX +
              originalStartX * originalWidth * scaleX;
            s.y =
              originalBBox.minY +
              translateY +
              originalStartY * originalHeight * scaleY;
            s.x2 =
              originalBBox.minX +
              translateX +
              originalEndX * originalWidth * scaleX;
            s.y2 =
              originalBBox.minY +
              translateY +
              originalEndY * originalHeight * scaleY;
            break;
          case "pencil":
            if (s.points && (originalS as any).points) {
              s.points = (originalS as any).points.map((p: any) => ({
                x:
                  originalBBox.minX +
                  translateX +
                  (p.x - originalBBox.minX) * scaleX,
                y:
                  originalBBox.minY +
                  translateY +
                  (p.y - originalBBox.minY) * scaleY,
              }));
            }
            break;
          case "text":
            s.fontSize = (originalS as any).fontSize * Math.max(scaleX, scaleY);
            break;
        }
        shape.timestamp = Date.now();
        shape.function = "move";
  ...
  private resizeLine(x: number, y: number) {
    ...
    if (
      this.selection.selectedShapes.length !== 1 ||
      !selectedShape ||
  ...
  private onMouseMove = (e: MouseEvent) => {
    ...
    if (this.selectedTool === Tools.Hand && this.canvasDrag.active) {
      ...
      if (this.selection.isMultiSelect) {
        ...
        if (
          this.selection.resizeHandle === "start" ||
          this.selection.resizeHandle === "end"
        ) {
    ...
    switch (this.selectedTool) {
      ...
      case Tools.Text:
        return {
          ...
          fontSize: 24 / this.current.scale, // Initial font size based on zoom
        };
    }
    return null;
  }
  ...
  private getLineResizeHandle(pos: { x: number; y: number }): string {
    ...
    if (Math.hypot(pos.x - line.x, pos.y - line.y) <= handleSize / 2)
      return "start";
    if (Math.hypot(pos.x - line.x2, pos.y - line.y2) <= handleSize / 2)
      return "end";
  ...
  private shapeBoundingBox() {
    if (
      this.selection.selectedShapes.length === 0 ||
      this.selection.isMultiSelect
    ) {
      return;
    }
